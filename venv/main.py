...
class VkUser:
    ...
    def __init__(self, token_vk, version):
        ...
        self.owner_id = requests.get(self.url + 'users.get', self.params).json()['responce'][0]['id']
    def get_photos(self, user_id=None, count=None, album_id='profile'):
        ...
        # получение url фотографии в максимальном размере
        photos_url_dict = {}
        for photo in res['response']['items']:
            photo_url = photo['sizes'][-1]['url']
            likes = photo['likes']['count']
            size = photo['sizes'][-1]['type']
            if likes not in photos_url_dict:
                photos_url_dict[likes] = {photo_url: size}
            else:
                photos_url_dict[f"{likes}_{datetime.today().strftime(f'%Y%m%d')}"] = {photo_url: size}
...
if __name__ == '__main__':
    upload = YaUploader(config.TOKEN_YA_DISK, config.TOKEN_VK, '5.131')
